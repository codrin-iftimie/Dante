...
(function() {
  ...
  Dante.Editor = (function(_super) {
    ...
    Editor.prototype.appendMenus = function() {
      var imageMenu, inlineTooltip, menu;
      menu = $("<div id='dante-menu' class='dante-menu'></div>").insertAfter(this.el);
      imageMenu = $("<div id='dante-image-menu' class='dante-menu'></div>").insertAfter(this.el);
      inlineTooltip = $("<div class='inlineTooltip'></div>").insertAfter(this.el);
      this.editor_menu = new Dante.Editor.Menu({
        el: menu,
        editor: this
      });
      this.image_menu = new Dante.Editor.ImageMenu({
        el: imageMenu,
        editor: this
      });
      ...
      this.image_menu.render();
      this.image_menu.hide();
    ...
    Editor.prototype.setRangeAfterBr = function() {
      var br, range, sel;
      sel = window.getSelection();
      range = sel.getRangeAt(0);
    ...
    Editor.prototype.displayMenu = function(sel) {
      return setTimeout((function(_this) {
        return function() {
          ...
          _this.relocateMenu(_this.editor_menu.$el, pos);
    ...
    Editor.prototype.hideMenues = function() {
      this.editor_menu.hide();
      return this.image_menu.hide();
    };

    Editor.prototype.relocateMenu = function(el, position) {
    ...
    Editor.prototype.handleGrafFigureSelectImg = function(ev) {
      ...
      this.selection().removeAllRanges();
      this.relocateMenu(this.image_menu.$el, element.getBoundingClientRect());
      this.image_menu.show($(element).parent(".graf--figure"));
      return this.editor_menu.hide();
    };
      ...
      if (e.which === ENTER) {
        ...
        if (anchor_node && this.editor_menu.lineBreakReg.test(anchor_node.nodeName)) {
          if (this.isLastChar()) {
            ...
            e.preventDefault();
            if (e.shiftKey) {
              if ($(".hack-br", parent).length === 0) {
                parent.append("<br class='hack-br'>");
              }
              this.setRangeAfterBr();
              parent.focus();
              return false;
            } else {
              this.handleLineBreakWith("p", parent);
...
(function() {
  ...
  Dante.Editor.ImageMenu = (function(_super) {
    __extends(ImageMenu, _super);

    function ImageMenu() {
      this.menuApply = __bind(this.menuApply, this);
      this.render = __bind(this.render, this);
      this.template = __bind(this.template, this);
      this.initialize = __bind(this.initialize, this);
      return ImageMenu.__super__.constructor.apply(this, arguments);
    }

    ImageMenu.prototype.events = {
      "mousedown li": "handleClick"
    };

    ImageMenu.prototype.initialize = function(opts) {
      if (opts == null) {
        opts = {};
      }
      this.current_editor = opts.editor;
    ...
    ImageMenu.prototype.default_config = function() {
      return {
    ...
    ImageMenu.prototype.template = function() {
      var html;
      html = "<ul class='dante-menu-buttons'>";
      _.each(this.config.buttons, function(item) {
        return html += "<li class='dante-menu-button'><i class=\"dante-icon icon-" + item + "\" data-action=\"" + item + "\">" + item + "</i></li>";
      });
      html += "</ul>";
      return html;
    };
    ...
    ImageMenu.prototype.handleClick = function(ev) {
      var action, element, input;
      element = $(ev.currentTarget).find('.dante-icon');
      action = element.data("action");
      input = $(this.el).find("input.dante-menu-input");
    ...
    ImageMenu.prototype.effectNode = function(el, returnAsNodeName) {
      var nodes;
      nodes = [];
      el = el || this.current_editor.$el[0];
      while (el !== this.current_editor.$el[0]) {
    ...
    ImageMenu.prototype.menuApply = function(action, value) {
      var MAX_WIDTH, classes_to_remove, height, img, ratio, width;
      classes_to_remove = this.config.buttons.map((function(_this) {
        return function(btn) {
          return "image-" + btn;
        };
      })(this));
      img = this.selectedImage.find("img");
      ...
      this.selectedImage.removeClass(classes_to_remove.join(" ")).addClass("image-" + action);
      return false;
    };

    ImageMenu.prototype.highlight = function(tag) {
      return $(".icon-" + tag).parent("li").addClass("active");
    };

    ImageMenu.prototype.show = function(elem) {
      this.selectedImage = elem;
      return $(this.el).addClass("dante-menu--active");
    };
